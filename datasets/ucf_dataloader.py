...
class UCF101DataLoader(Dataset):
    ...
    def get_det_annots_prepared(self, file_id, percent='40'):
        ...
        # training_annot_file = "../training_annots.pkl"
        # training_annot_file = "../train_annots_"+ percent + "_per.pkl"
    ...
    def __getitem__(self, index):
        ...
        if len(annot_frames) == 1:
            ...
            if len(annot_frames) <= 0:
                ...
                label_cls = torch.from_numpy(label_cls)
                # print("---------NOBBOXERR-----------")
                sample = {'data':video_rgb,'segmentation':label_cls,'action':torch.Tensor([0]), 'flip_data': video_rgb, 'flip_label': label_cls, 'labeld_vid':labeled_vid}
                return sample
        ...
        # sample = {'data':video_rgb,'segmentation':label_cls,'action':action_tensor,'sup_label': vid_lab_unlab}
        ...
        # sample = {'vid_name':v_name}
        # sample.get(video_label_avail)
