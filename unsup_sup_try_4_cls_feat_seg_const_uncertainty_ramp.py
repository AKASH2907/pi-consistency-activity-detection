...
def train_model_interface(args, label_minibatch, unlabel_minibatch, wt_ramp):
    ...
    # ramping of weighted and unweighted loss
    total_seg_cons_loss = (wt_ramp * loss_wt_2) + ((1 - wt_ramp) * loss_wt_1)

    # total_seg_cons_loss = loss_wt_2
...
def train(args, model, labeled_train_loader, unlabeled_train_loader, optimizer, scheduler, epoch, r, save_path, writer, ramp_wt, short=False):
    ...
    for batch_id, (label_minibatch, unlabel_minibatch)  in enumerate(zip(cycle(labeled_train_loader), unlabeled_train_loader)):
        ...
        output, predicted_action, segmentation, action, loss, s_loss, c_loss, const_loss, cc_loss, sc_loss = train_model_interface(args, label_minibatch, unlabel_minibatch, ramp_wt(epoch))
        ...
        if (batch_id + 1) % args.pf == 0:
            ...
            print(f'[TRAIN] epoch-{epoch}/{N_EPOCHS}, batch-{batch_id+1}/{steps}, loss-{r_total:.3f}, acc-{r_acc:.3f}' \
                f'\t [LOSS ] cls-{r_class:.3f}, seg-{r_seg:.3f}, const-{r_const:.3f}, cls_const-{r_cls_const:.3f}, seg_const-{r_seg_const:.3f}')
            # summary writing
...
if __name__ == '__main__':
    ...
    # ramp_wt = ramp_ups.linear_rampup(N_EPOCHS)
    ...
    for e in tqdm(range(1, N_EPOCHS + 1)):
        ...
        r, train_loss = train(args, model, labeled_train_data_loader, unlabeled_train_data_loader, optimizer, scheduler, e, r, save_path, writer, ramp_wt, short=False)
