...
def val_model_interface(minibatch, wt_seg, wt_cons_cls, const_loss, r=0):
    data = minibatch['data']
    ...
    if const_loss == 'jsd':
        ...
        seg_cons_loss = consistency_criterion(flipped_pred_seg_map, flip_op)
        total_cons_loss = cons_loss_a + cons_loss_b + seg_cons_loss


    elif const_loss == 'l1':
        consistency_criterion = nn.L1Loss()
        total_cons_loss = consistency_criterion(flipped_pred_seg_map, flip_op)
...
def train_model_interface(label_minibatch, unlabel_minibatch, wt_seg, wt_cons_cls, const_loss, r=0):
    ...
    if const_loss == 'jsd':
        ...
        seg_cons_loss = seg_consistency_criterion(flipped_pred_seg_map, flip_op)
        total_cons_loss = cons_loss_a + cons_loss_b + seg_cons_loss
...
def train(model, labeled_train_loader, unlabeled_train_loader, optimizer, epoch, r, save_path, writer, wt_seg, wt_cons_cls, const_loss, short=False):
...
def validate(model, val_data_loader, epoch, wt_seg, wt_cons_cls, const_loss, short=False):
    ...
    with torch.no_grad():
        ...
        for batch_id, minibatch in enumerate(val_data_loader):
            ...
            output, predicted_action, segmentation, action, loss, s_loss, c_loss, cc_loss = val_model_interface(minibatch, wt_seg, wt_cons_cls, const_loss, r)
            total_loss.append(loss.item())
            seg_loss.append(s_loss.item())
            class_loss.append(c_loss.item())
            class_consistency_loss.append(cc_loss.item())
...
if __name__ == '__main__':
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', min_lr=1e-7, patience=10, factor=0.1)
    ...
    for e in tqdm(range(1, N_EPOCHS + 1)):
        ...
        r = train(model, labeled_train_data_loader, unlabeled_train_data_loader, optimizer, e, r, save_path, writer, args.wt_seg, args.wt_cons_cls, args.const_loss, short=False)
