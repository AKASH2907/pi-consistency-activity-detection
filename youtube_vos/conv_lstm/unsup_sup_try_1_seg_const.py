...
def train(args, model, labeled_dloader, unlabeled_dloader, epoch, criterion, optimizer):
    model.train()
    running_loss = 0.0
    running_acc = 0.0
    losses, accs = [], []
    ...
    for i, (labeled_sample, unlabeled_sample) in enumerate(zip(cycle(labeled_dloader), unlabeled_dloader)):
        ...
        concat_labels = concat_labels[random_indices]      # THIS LINE WAS IMP FORGOT THIS BEFORE --> KEEP A CHECK
...
def run_experiment():
    ...
    # for epoch in range(1, args..n_epochs + 1):
        ...
        loss, acc = train(args, model, labeled_dloader, unlabeled_dloader, epoch, criterion, optimizer, )
