...
def train(args, model, epoch, labeled_dloader, unlabeled_dloader, criterion, optimizer, ramp_wt):
    model.train()
    running_loss = 0.0
    running_acc = 0.0
    losses, accs = [], []
    ...
    for i, (labeled_sample, unlabeled_sample) in enumerate(zip(cycle(labeled_dloader), unlabeled_dloader)):
        ...
        equal_wt = torch.ones_like(y_pred_logits, dtype=torch.double)
        equal_wt = equal_wt.type(torch.cuda.FloatTensor)
        loss_wt_1 = weighted_mse_loss(y_pred_logits_fl, y_pred_logits, equal_wt)
        ...
        loss_wt_2 = weighted_mse_loss(y_pred_logits_fl, y_pred_logits, batch_variance_orig_clip)

        # REDUCE NORMAL L2 and improvise WEIGHTED L2
        total_cons_loss = (ramp_wt(epoch) * loss_wt_2) + ((1 - ramp_wt(epoch)) * loss_wt_1)
        

        loss = seg_loss + args.wt_cons * total_cons_loss
...
def run_experiment():
    ...
    labeled_json_file = 'meta_10percent_labeled.json'
    unlabeled_json_file = 'meta_90percent_unlabeled.json'
    ...
    ramp_wt = ramp_ups.exp_rampup(args.epochs)
    ...
    # for epoch in range(1, args..n_epochs + 1):
        ...
        loss, acc = train(args, model, epoch, labeled_dloader, unlabeled_dloader, criterion, optimizer, ramp_wt)
