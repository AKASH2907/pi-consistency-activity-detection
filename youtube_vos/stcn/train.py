...
# REMOVE SIMPLE IF NEED AUG
from dataset.vos_dataset_simple import VOSDataset

from util.logger import TensorboardLogger
from util.hyper_para import HyperParameters
from util.load_subset import load_sub_yv, load_sub_yv_labeled
...
try:
    for e in range(current_epoch, total_epoch): 
        ...
        # Train loop
        model.train()
        # as i understand the result is flushed after every 100 iterations
        # if 10% dataset=347, then 1 epoch has roughly 21 iterations
        # so it will be printed after every 4 to 5 epochs
        # increase the lr with batch size
