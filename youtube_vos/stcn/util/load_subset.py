...
# def load_sub_yv(path='util/yv_subset_20percent_labeled.txt'):
    with open(path, mode='r') as f:
        subset = set(f.read().splitlines())
    return subset


def load_sub_yv_labeled(path='util/yv_subset_10percent_labeled.txt'):
    with open(path, mode='r') as f:
        subset = set(f.read().splitlines())
    return subset

def load_sub_yv_unlabeled(path='util/yv_subset_90percent_unlabeled.txt'):
    with open(path, mode='r') as f:
        subset = set(f.read().splitlines())
    return subset