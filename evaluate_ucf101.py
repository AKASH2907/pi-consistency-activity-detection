...
def iou(split, debug=False):
    ...
    filtered_files = [file.split("/")[-1] for file in glob.glob(osp.join(args.ckpt, 'best_model_' + split + '*.pth'))]

    for saved_wts in sorted(glob.glob(osp.join(args.ckpt, 'best_model_' + split + '*.pth'))):
        ...
        with torch.no_grad():
            ...
            validationset = UCF101DataLoader('validation',[224, 224], 1, file_id=pkl_file_label , use_random_start_frame=False)
