...
def val_model_interface(minibatch, wt_seg, wt_cons_cls, r=0):
    data = minibatch['data']
...
def train_model_interface(label_minibatch, unlabel_minibatch, wt_seg, wt_cons_cls, r=0):
    ...
    # print(feat.shape, flip_feat.shape, predicted_action.shape)
...
def train(model, labeled_train_loader, unlabeled_train_loader, optimizer, epoch, r, save_path, writer, wt_seg, wt_cons_cls, short=False):
    ...
    for batch_id, (label_minibatch, unlabel_minibatch)  in enumerate(zip(cycle(labeled_train_loader), unlabeled_train_loader)):
        ...
        output, predicted_action, segmentation, action, loss, s_loss, c_loss, cc_loss = train_model_interface(label_minibatch, unlabel_minibatch, wt_seg, wt_cons_cls, r)
...
def validate(model, val_data_loader, epoch, wt_seg, wt_cons_cls, short=False):
    ...
    with torch.no_grad():
        ...
        for batch_id, minibatch in enumerate(val_data_loader):
            ...
            output, predicted_action, segmentation, action, loss, s_loss, c_loss, cc_loss = val_model_interface(minibatch, wt_seg, wt_cons_cls, r)
            total_loss.append(loss.item())
            seg_loss.append(s_loss.item())
            class_loss.append(c_loss.item())
            class_consistency_loss.append(cc_loss.item())
...
if __name__ == '__main__':
    ...
    for e in tqdm(range(1, N_EPOCHS + 1)):
        ...
        r = train(model, labeled_train_data_loader, unlabeled_train_data_loader, optimizer, e, r, save_path, writer, args.wt_seg, args.wt_cons_cls, short=False)

        val_loss = validate(model, val_data_loader, e, args.wt_seg, args.wt_cons_cls, short=False)
