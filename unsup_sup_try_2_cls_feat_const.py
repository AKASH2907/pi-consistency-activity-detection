...
#####################################################
'''
train_log_wts - 10/90 randomly shuffled
...
def train_model_interface(args, label_minibatch, unlabel_minibatch, ramp_wt):
...
def train(args, model, labeled_train_loader, unlabeled_train_loader, optimizer, epoch, r, save_path, writer, iterations, short=False):
    ...
    for batch_id, (label_minibatch, unlabel_minibatch)  in enumerate(zip(cycle(labeled_train_loader), unlabeled_train_loader)):
        ...
        ramp_wt = ramps.rampweight(iterations)
        iterations += 1
        output, predicted_action, segmentation, action, loss, s_loss, c_loss, cc_loss = train_model_interface(args, label_minibatch, unlabel_minibatch, ramp_wt)
    ...
    return r, iterations, train_total_loss
...
def parse_args():
    ...
    parser.add_argument('--pkl_file_unlabel', type=str, default='train_annots_90_unlabeled_random.pkl', help='experiment name')
...
if __name__ == '__main__':
    ...
    unlabeled_trainset = UCF101DataLoader('train', [224, 224], batch_size=4, file_id=args.pkl_file_unlabel, percent=percent, use_random_start_frame=False)
    ...
    for e in tqdm(range(1, N_EPOCHS + 1)):
        r, iters, train_loss = train(args, model, labeled_train_data_loader, unlabeled_train_data_loader, optimizer, e, r, save_path, writer, iterations, short=False)
        iterations = iters
